...
@Component({
  selector: 'app-mes-validations-list',
  templateUrl: './mes-validations-list.component.html',
  styleUrls: ['./mes-validations-list.component.css']
})
export class MesValidationsListComponent {
  ...
  constructor(private formBuilder:FormBuilder,
    ...
    validerAnnuler(affectation:any, etat:number, title :string){
      ...
      this._utilisateurService.updateEtat(affectation, etat).subscribe((data) => {
          ...
          this.router.navigate(['/mes-validations-list', this.id_utilisateur]);
    ...
    annulerValidation(affectation:any, etat:number){
      
      let title : string = "Validation annulée avec succès"
    ...
    edit(affectation:any){
      ...
      this.router.navigate(['/assigner-mission-edit',this.id_utilisateur, affectation.id_affectation, this.administration]);
        ...
        this._utilisateurService.getProfilById(this.id_profil).subscribe((data:any)=>{
          ...
          for (let index = 0; index < this.affectations.length; index++) {
            ...
            if (Number(this.administration) === 1){
              ...
              if((Number(element.id_utilisateur) === Number(this.id_utilisateur)) && Number(element.statut) === 1){
                this.resume_duree += Number(element.duree)        
                this.resume_montant_option += Number(element.montant_option )    
                this.resume_montant += Number(element.montant)        
                this.resume_total_facture += Number(element.total_facture)          
